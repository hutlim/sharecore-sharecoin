...
*Setup instructions for a Gitian build of Sharecoin using a Debian VM or physical system.*
...
- [Building Sharecoin](#building-sharecoin)
...
Clone the git repositories for sharecoin and Gitian and then checkout the sharecoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the sharecoin repository
...
To build Sharecoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the sharecoin repository.
...
For example:
```bash
URL=https://github.com/hutlim/sharecoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sharecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sharecoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/sharecoin-build.assert
```
...
[sharecoin/gitian.sigs](https://github.com/hutlim/gitian.sigs.shc/) repository, or if that's not possible create a pull
request. 
